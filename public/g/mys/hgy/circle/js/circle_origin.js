...
try {
   ...
   /* $.ajax({
    ...
    alert('sorry , it is not supported');
}
// circle related
...
var versionText = new PointText({
    ...
    content: 'the best game is in the game kingdom！'
});
...
helpText.content = 'Draw a circle\n\n\nClick Here to Start';
...
tutorialText.content = 'more games';
...
function encourage(perfection) {
    if (perfection < 0.1) {
        return 'Wow,It is piece of shit!';
    } else if (perfection < 0.5) {
        return 'Do you think it is a circle?';
    } else if (perfection < 0.7) {
        return 'The circle looks strange';
    } else if (perfection < 0.8) {
        return 'The circle is carelessly！';
